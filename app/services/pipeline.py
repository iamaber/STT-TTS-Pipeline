...
class Pipeline:
    ...
    def process_text_to_audio(self, text: str, speaker: Optional[int] = None) -> np.ndarray:
        audio = self.tts.synthesize(text, speaker)
        return audio
    
    def process_full_pipeline(self, input_audio: np.ndarray, 
                            input_sr: int = 16000,
                            speaker: Optional[int] = None) -> tuple[str, np.ndarray]:
