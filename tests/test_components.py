...
def test_asr():
    ...
    try:
        asr = ASRModel(model_path=settings.asr.model_name, device=settings.asr.device)
...
def test_tts():
    ...
    try:
        ...
        print(
            f"✓ TTS test passed. Generated {len(audio)} samples ({duration:.2f}s) at {tts.tts.sample_rate}Hz"
        )
        return True
    except Exception as e:
